...
class Salma_users_table extends CI_Model
{
    private $db_main;
    ...
    public function repair_table()
    {
        ...
        $aSql=array();
        $table= $this->db_main->dbprefix($this->table );
        if(!$this->db_main->field_exists('u_currency', $this->table)){
                $aSql[]="ALTER TABLE `$table` ADD `u_currency` VARCHAR(11) default 'IDR';";
        }
